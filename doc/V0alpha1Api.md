...
final api = KratosApi().getV0alpha1Api();
...
try { 
    final response = api.adminGetIdentity(id);
    print(response);
} catch on DioError (e) {
...
try { 
    final response = api.initializeSelfServiceLoginFlowForBrowsers(refresh);
    print(response);
} catch on DioError (e) {
...
try { 
    final response = api.initializeSelfServiceLoginFlowWithoutBrowser(refresh);
    print(response);
} catch on DioError (e) {
...
try { 
    final response = api.initializeSelfServiceRecoveryFlowForBrowsers();
    print(response);
} catch on DioError (e) {
...
try { 
    final response = api.initializeSelfServiceRegistrationFlowForBrowsers();
    print(response);
} catch on DioError (e) {
...
final api = KratosApi().getV0alpha1Api();
final String cookie = cookie_example; // String | The Session Cookie of the Identity performing the settings flow.

try { 
    final response = api.initializeSelfServiceSettingsFlowForBrowsers(cookie);
    print(response);
} catch on DioError (e) {
...
try { 
    final response = api.initializeSelfServiceVerificationFlowForBrowsers();
    print(response);
} catch on DioError (e) {
...
try { 
    final response = api.submitSelfServiceLoginFlow(flow, submitSelfServiceLoginFlowBody);
    print(response);
} catch on DioError (e) {
